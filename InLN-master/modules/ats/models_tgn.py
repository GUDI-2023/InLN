...
from modules.ats.adaptive_span import AdaptiveSpan
from modules.ats.persistent_memory import PersistentMemory
from modules.ats.adaptive_io import build_adaptive_io, compute_dummy_loss
...
class SeqAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_pe, attn_mask=None):
        ...
        if attn_mask is not None:
            ...
            attn.masked_fill_(attn_mask.bool(), -1e9)
...
class MultiHeadSeqAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_pe, attn_mask=None):
        ...
        if attn_mask!=None:  # attn_mask: [b_size x len_q x len_k]
            attn_mask = attn_mask.unsqueeze(1).repeat(1, self.nb_heads, 1, 1).view(-1, attn_mask.size(-2), attn_mask.size(-1))
        ...
        out = self.attn(query, key, value, key_pe, attn_mask)  # B_K x M x D
...
class TransformerSeqLayer(nn.Module):
    ...
    def forward(self, h, h_cache, key_pe, self_attn_mask):
        # h = B x M x H
        # h_cache = B x L x H
        # h_all = torch.cat([h_cache, h], dim=1)  # B x (M+L) x H
        #print(h.dtype, key_pe.dtype, self_attn_mask.dtype)
        attn_out = self.attn(h, h, h, key_pe, self_attn_mask)
...
class TransformerSeq(nn.Module):
    def __init__(self, vocab_size, hidden_size, nb_heads, nb_layers,
        ...
        if self.adapt_io:
            ...
            self.out_emb = nn.Linear(hidden_size, vocab_size) # 输入，输出
        if emb_dropout > 0:
    ...
    def forward(self, h, self_attn_mask):
        # x 是list , 每一个值 是一个tensor， （message length, message emb dim）

        #h = self.in_emb(x)  # 输出：B x M x H: M 是句的长度

        if self.emb_dropout is not None:
            h = self.emb_dropout(h)

        for l, layer in enumerate(self.layers):
            h = layer(h, h, self.key_pe, self_attn_mask)  # B x M x H
        ...
        if self.adapt_io:
            ...
            #loss = self.out_emb(h, target)
            dummy_loss = compute_dummy_loss(self.in_emb, self.out_emb)
        else:
            out = self.out_emb(h)
            #print (out,out.size())
            #loss = F.log_softmax(self.out_emb(h), dim=-1)
            #dummy_loss = None
