...
class AdaptiveMask(nn.Module):
    ...
    def __init__(self, max_size, ramp_size, init_val=0, shape=(1,)):
        ...
        mask_template = torch.linspace(1 - max_size, 0, steps=max_size) # 返回一个一维的tensor(start, end, 分割点数)
...
class AdaptiveSpan(nn.Module):
    ...
    Args:
        attn_span: maximum attention span 最大的attn span的长度
        adapt_span_loss: loss coefficient for the span length
        adapt_span_ramp: length of the masking ramp 攀升部分的长度
        adapt_span_init: initial size ratio 这是一个比例，百分之多少
    ...
    def forward(self, attn, normalize=True):
        ...
        attn = attn.reshape(B // self._nb_heads, self._nb_heads, M, -1) #转换维度
