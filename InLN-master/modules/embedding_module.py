...
# 新建一个graph然后执行temporal random walk : 1, 建图， 2， randow walk 执行 ，返回path, 3. 何时执行？
...
class GraphEmbedding(EmbeddingModule):
  ...
  def compute_embedding(self, memory, source_nodes, timestamps, n_layers,sou_neighbor, des_neighbor, n_neighbors=20, time_diffs=None,
                        use_time_proj=True):
    ...
    if n_layers == 0:
      ...
      if type(sou_neighbor)==int:
        ...
        neighbors, edge_idxs, edge_times = self.neighbor_finder.get_ada_neighbor2(source_nodes, timestamps,sou_neighbor, des_neighbor, n_neighbors=n_neighbors)
      ...
      neighbor_embeddings = self.compute_embedding(memory,
                                                   neighbors,
                                                   np.repeat(timestamps, n_neighbors),sou_neighbor=sou_neighbor, des_neighbor=des_neighbor,
                                                   n_layers=n_layers - 1,
